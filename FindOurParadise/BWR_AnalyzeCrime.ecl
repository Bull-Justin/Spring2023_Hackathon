...
Crime    := Datasets.File_Crimes.File;
CrimeRec := Datasets.File_Crimes.Layout;
...
RatioRec CalcRatios(AveCrimeTBL Le) := TRANSFORM
 ...
 // Scale with weights compared ot Homicide and Rape
 ...
 SELF.Agg_AssaultRat    := ((Le.Agg_AssaultAvg /Le.PopAvg  * 100 * 0.8));
 SELF.ViolentCompRat    := (((Le.ViolentCrimeAvg + Le.HomicideAvg + Le.RapeAvg + Le.Agg_AssaultAvg)/4)/Le.PopAvg) * 100;
 // Scale with weights compared to Homicide and Rape
...
OUTPUT(BuildRatios,,'~FYP::Main::Hacks::TeamFriendshipCrimeRates',NAMED('CrimeRatiosByPopulation'),OVERWRITE);
