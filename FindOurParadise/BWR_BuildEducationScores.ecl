...
AddSTScore  := ITERATE(SORT(TempTbl,-avestratio),
                            TRANSFORM(RankTbl,
                                      SELF.StudentTeacherScore := IF(LEFT.avestratio=RIGHT.avestratio,
                                                    LEFT.StudentTeacherScore,LEFT.StudentTeacherScore+1),
                                      SELF := RIGHT));
AddPrvScore := ITERATE(SORT(AddSTScore,prvcnt), 
                        TRANSFORM(RankTbl,
                          SELF.PrvSchoolScore := IF(LEFT.prvcnt=RIGHT.prvcnt,
                                            LEFT.PrvSchoolScore,LEFT.PrvSchoolScore+1),
                                            SELF := RIGHT));
AddPubScore := ITERATE(SORT(AddPrvScore,pubcnt),
                        TRANSFORM(RankTbl,
                          SELF.PublicSchoolScore := IF(LEFT.pubcnt=RIGHT.pubcnt,
                                                        LEFT.PublicSchoolScore,
                                                        LEFT.PublicSchoolScore+1),
                          SELF := RIGHT));
